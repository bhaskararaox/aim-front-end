...
export class BySegmentController {
  private dataService: TraceDataService;
  public messageService: MessageService;
  private searchData: SearchData;
  public data: Summary[];
  public showBySegment: boolean;
  ...
  public load() {
    ...
    this.dataService.getSummaryBySegment(this.searchData.input).subscribe(searchResult => {
      this.dataTableReady = true;
      this.loadTable(searchResult);
      this.loadChart(searchResult);
      this.showBySegment = true;
  ...
  private loadTable(searchResult: any) {
    for (let row of searchResult.data) {
      this.data.push(new Summary(row.segmentName, row.numberOfTraces));
  ...
  private loadChart(searchResult: any) {
    ...
    for (let row of searchResult.data) {
      chartData.push(new Summary(row.segmentName, row.numberOfTraces));
    }
    ...
    this.elementIdSummary = "segmentPieChart";
  }
